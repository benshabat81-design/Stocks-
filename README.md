...
אפשר לנעול את אפליקציית ה-Web בסיסמה כך שרק מי שמכיר אותה ייכנס.
ב-Streamlit Community Cloud: **Settings → Secrets**, והוסף שורה אחת:
```toml
app_password = "הסיסמה-שלך"
```
אם לא מגדירים `app_password`, האפליקציה נפתחת ללא סיסמה.

### ניתוח חדשות חכם ב-AI (אופציונלי, חינמי)
בלשונית **סורק שוק → חדשות אחרונות** אפשר להפעיל ניתוח חכם בעברית לכל כתבה
(טיקרים קשורים, סנטימנט, סיכום, המלצה וציון דחיפות 1–5) באמצעות **Google Gemini** —
שכבה חינמית לגמרי (ללא כרטיס אשראי).
מפיקים מפתח חינם ב-[aistudio.google.com](https://aistudio.google.com/app/apikey),
ומוסיפים ב-**Settings → Secrets**:
```toml
GEMINI_API_KEY = "AIza..."
# אופציונלי - לבחור מודל אחר:
# GEMINI_MODEL = "gemini-2.0-flash"
```
בלי המפתח, האפליקציה עובדת רגיל ומציגה רמז קצר איך להפעיל.
